...
export class Heimdall {
  constructor(config, options = {}) {
    ...
    this.repositoryUrl = options.repositoryUrl || 'https://github.com/Ramestta-Blockchain/heimdall'
  }
    ...
    return new Listr([
      ...
      },
      {
        ...
        task: () => {
          ...
          fileReplacer(this.heimdallGenesisFilePath)
            .replace(/"matic_token_address":[ ]*".*"/gi, `"matic_token_address": "${rootContracts.tokens.MaticToken}"`)
            .replace(/"staking_manager_address":[ ]*".*"/gi, `"staking_manager_address": "${rootContracts.StakeManagerProxy}"`)
            .replace(/"slash_manager_address":[ ]*".*"/gi, `"slash_manager_address": "${rootContracts.SlashingManager}"`)
